...
import { startButtonPaymentAction } from "./actions";

export const dynamic = "force-dynamic";

export default async function PayButtonPage({ params }: { params: Promise<{ businessId: string }> }) {
  ...
  const business = await db.business.findUnique({
    ...
    include: {
      paymentButtons: { where: { active: true }, take: 1 },
    },
  });
  if (!business) notFound();

  const button = business.paymentButtons[0];
  const fixedAmount = button?.amount ? Number(button.amount) : null;
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          {button?.description && (
            <p className="mt-1 text-sm text-slate-500">{button.description}</p>
          )}
          {!button?.description && (
            <p className="mt-1 text-sm text-slate-500">{button?.label ?? "Complete your payment"}</p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-white/80 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur-sm">
          ...
          <form action={startButtonPaymentAction} className="grid gap-4">
            <input type="hidden" name="businessSlug" value={business.slug} />
            {button?.id && <input type="hidden" name="paymentButtonId" value={button.id} />}
            {fixedAmount && <input type="hidden" name="amount" value={String(fixedAmount)} />}
            ...
            {fixedAmount && (
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
                <p className="text-xs font-medium text-blue-600">Amount to pay</p>
                <p className="text-2xl font-bold text-blue-900">
                  ₦{fixedAmount.toLocaleString("en-NG")}
                </p>
              </div>
            )}
