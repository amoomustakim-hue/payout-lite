...
export async function startShopPaymentAction(formData: FormData) {
  ...
  const reference = `pl_shop_${business.id.slice(0, 8)}_${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`;

  const transaction = await db.transaction.create({
    data: {
      businessId: business.id,
      source: TransactionSource.SHOP_QR,
  ...
  if (missing.length > 0) {
    ...
    redirect(`/pay/shop/${businessSlug}/error?reference=${reference}&reason=nomba-config`);
  }
  ...
  try {
    const order = await createNombaCheckoutOrder({
      ...
      callbackUrl: `${getAppUrl()}/pay/shop/${businessSlug}/pending?reference=${reference}`,
      description: `Shop payment to ${business.name}`,
      subAccountId: business.nombaSubAccountId,
      metadata: { transactionId: transaction.id, source: TransactionSource.SHOP_QR },
    });
    ...
    redirect(`/pay/shop/${businessSlug}/error?reference=${reference}&reason=unavailable`);
  }

  if (!checkoutUrl || !isHttpUrl(checkoutUrl)) {
    redirect(`/pay/shop/${businessSlug}/error?reference=${reference}&reason=missing-url`);
  }
